...
export function useSocket(token: string | null): Socket {
  ...
  useEffect(() => {
    ...
    function identify() { socket.emit('client:identify', { role: 'manager' }); }
    socket.auth = { token };
    // Re-identify on every (re)connect — socket.io auto-reconnects after
    // network blips / app backgrounding; without this we'd silently drop out
    // of the 'manager' room and stop receiving ticket:updated.
